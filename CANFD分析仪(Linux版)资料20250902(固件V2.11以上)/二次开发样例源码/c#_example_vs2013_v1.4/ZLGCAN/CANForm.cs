    ...
    public partial class CANForm : Form
    {
        ...
        private void button_init_Click(object sender, EventArgs e)
        {
            ...
            if (!m_bCloud)
            {
                ...
                property_ = (IProperty)Marshal.PtrToStructure((IntPtr)(ptr), typeof(IProperty));
        ...
        private void button_reset_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("复位CAN1失败", "提示", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            //CAN2
            if (Method.ZCAN_ResetCAN(channel_handle2_) != Define.STATUS_OK)
            {
                MessageBox.Show("复位CAN2失败", "提示", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
        ...
        */
        ...
        private void AddData(ZCAN_Receive_Data[] data, uint len, uint ch)
        {
            ...
            for (uint i = 0; i < len; ++i)
            {
                ...
                if (ch == 0)
                {
                    list_box_data_ = String.Format("[通道0] 接收到CAN ID:0x{0:X8} {1:G} {2:G} 长度:{3:D1} 数据:", GetId(id), eff, rtr, can.frame.can_dlc);
                }
                else
                {
                    list_box_data_ = String.Format("[通道1] 接收到CAN ID:0x{0:X8} {1:G} {2:G} 长度:{3:D1} 数据:", GetId(id), eff, rtr, can.frame.can_dlc);
                }
        ...
        private void AddData(ZCAN_ReceiveFD_Data[] data, uint len, uint ch)
        {
            ...
            for (uint i = 0; i < len; ++i)
            {
                ...
                if (ch == 0)
                {
                    list_box_data_ = String.Format("[通道0] 接收到CANFD ID:0x{0:X8} {1:G} {2:G} 长度:{3:D1} 数据:", GetId(id), eff, rtr, canfd.frame.len);
                }
                else
                {
                    list_box_data_ = String.Format("[通道1] 接收到CANFD ID:0x{0:X8} {1:G} {2:G} 长度:{3:D1} 数据:", GetId(id), eff, rtr, canfd.frame.len);
                }
