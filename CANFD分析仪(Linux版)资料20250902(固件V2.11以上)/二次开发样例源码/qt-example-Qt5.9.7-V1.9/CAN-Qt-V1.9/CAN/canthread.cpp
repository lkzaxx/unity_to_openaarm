...
bool CANThread::sendData(UINT id,UINT frame_type_index,UINT protocol_index,UINT canfd_exp_index,UINT channel,const char *data,UINT len)
{
    ...
    else //canfd
    {
        ...
        canfd_data.transmit_type = 0;//单次发送
...
{
    UINT frameCount = 0;
    ZCAN_Receive_Data recvCANData[1000];
    ZCAN_ReceiveFD_Data recvCANFDData[1000];
    while(!stopped)
    {
        //CAN1
        ...
        //if(frameCount > 0)
        {
            frameCount = ZCAN_Receive(m_channel1,recvCANData,1000,0);//(frameCount > 200 ? 200 : frameCount),50);
        ...
        //if(frameCount > 0)
        {
            frameCount = ZCAN_ReceiveFD(m_channel1,recvCANFDData,1000,0);//,(frameCount > 200 ? 200 : frameCount),50);
        ...
        //if(frameCount > 0)
        {
            frameCount = ZCAN_Receive(m_channel2,recvCANData,1000,0);//(frameCount > 200 ? 200 : frameCount),50);
        ...
        //if(frameCount > 0)
        {
            frameCount = ZCAN_ReceiveFD(m_channel2,recvCANFDData,1000,0);//(frameCount > 200 ? 200 : frameCount),50);
