...
void *receive_func(void *param) // 接收线程。
{
    ...
    ZCAN_Receive_Data zcan_receive_data[3000];
    int can_index = 0;
    ...
    while ((*run) & 0x0f)
    {
		...
		printf_len = 35;
		    ...
		    printf_len += 10;
                ...
                if (zcan_receive_fd_data[i].frame.flags == 1)
                {
                    printf(" 加速 ");
                    printf_len += 8;
                }
		...
		printf_len += 8;
			...
			printf("\n");
                ...
                printf(" TimeStamp:0x%08X", zcan_receive_fd_data[i].timestamp); // 时间标识。
                printf("\n");
            }
        }
            //can
        real_len = zcan_receive(m_h_channel[device_index][can_index], zcan_receive_data, 3000, wait_time);
        if (real_len > 0)
        {
	    int printf_len = 0;
            for (int i = 0; i < real_len; i++)
            {
                ...
                printf("CAN%d RX ID:0x%08X", can_index + 1, GET_ID(zcan_receive_data[i].frame.can_id));
		printf_len = 35;
                if (IS_EFF(zcan_receive_data[i].frame.can_id) == 0)
                {
                    // 帧格式：标准帧
                    printf(" Standard ");
		    printf_len += 10;
                }
                if (IS_EFF(zcan_receive_data[i].frame.can_id) == 1)
                {
                    // 帧格式：扩展帧
                    printf(" Extend   ");
	            printf_len += 10;
                }
                if (IS_RTR(zcan_receive_data[i].frame.can_id) == 0)
                {
                    // 帧类型：数据帧
                    printf(" Data   ");
                    printf_len += 10;
                }
                if (IS_RTR(zcan_receive_data[i].frame.can_id) == 1)
                {
                    printf(" Remote ");
                    printf_len += 8;
                }
                printf("DLC:0x%02X", zcan_receive_data[i].frame.can_dlc);
		printf_len += 8;
                printf(" data:(0x)"); // 数据
                printf_len += 8;
                for (int j = 0; j < zcan_receive_data[i].frame.can_dlc; j++)
                {
		    
                    printf(" %02X", zcan_receive_data[i].frame.data[j]);
	            if ((j + 1) % 8 == 0 )
		    {
			printf("\n");
                        printf("%*s", printf_len, "");
                ...
                printf(" TimeStamp:0x%08X", zcan_receive_data[i].timestamp); // 时间标识。
                printf("\n");
            }
        }
        can_index = !can_index;
...
int main(int, char **)
{
    std::cout << "Hello, this is canfd test demo!\n";
    ...
    zcan_set_abit_baud(dev_handle,0,1000000);
    zcan_set_dbit_baud(dev_handle,0,5000000);
    zcan_set_abit_baud(dev_handle,1,1000000);
    zcan_set_dbit_baud(dev_handle,1,5000000);
    ...
    //can
    ZCAN_Transmit_Data zcan_transmit_data[message_len];
    eff = 0;
    rtr = 0;
    id = 0x00;
    zcan_transmit_data[0].frame.can_id = MAKE_CAN_ID(id, eff, rtr, 0);
    zcan_transmit_data[0].frame.can_dlc = 8;
    memset(zcan_transmit_data[0].frame.data, 0, sizeof(zcan_transmit_data[0].frame.data));
    for (int i = 0; i < 8; i++)
    {
        zcan_transmit_data[0].frame.data[i] = i;
    }
    int times = 1;
    int printf_len = 0;
    ...
    while (times--)
    {
        ...
        if (zcan_transmit_fd(m_h_channel[device_index][0], zcan_transmitfd_data, message_len) == 1)
        {
            printf("Index:%04d  ", message_count);
            message_count++;
            printf("CANFD1加速 TX ID:0x%08X", id);
	    printf_len = 34;
        ...
        if (zcan_transmit_fd(m_h_channel[device_index][1], zcan_transmitfd_data, message_len) == 1)
        {
            printf("Index:%04d  ", message_count);
            message_count++;
            printf("CANFD2加速 TX ID:0x%08X", id);
            if (IS_EFF(zcan_transmitfd_data[0].frame.can_id) == 0)
            {
                printf(" Standard ");
            }
    ...
    //fd
    zcan_transmitfd_data[0].frame.flags = 0;
    times = 1;
    printf_len = 0;
    while (times--)
    {
    ...
    while (times--)
    {
        if (zcan_transmit(m_h_channel[device_index][0], zcan_transmit_data, message_len) == 1)
        {
            printf("Index:%04d  ", message_count);
            message_count++;
            printf("CAN1 TX ID:0x%08X", id);
	    printf_len = 34;
            if (IS_EFF(zcan_transmit_data[0].frame.can_id) == 0)
            {
                printf(" Standard ");
                printf_len += 10;
            }
            if (IS_EFF(zcan_transmit_data[0].frame.can_id) == 1)
            {
                printf(" Extend   ");
		printf_len += 10;
            }
            if (IS_RTR(zcan_transmit_data[0].frame.can_id) == 0)
            {
                printf(" Data   ");
		printf_len += 10;
            }
            if (IS_RTR(zcan_transmit_data[0].frame.can_id) == 1)
            {
                printf(" Remote ");
		printf_len += 9;
            }
            printf("DLC:0x%02X", zcan_transmit_data[0].frame.can_dlc);
	    printf_len += 8;
            printf(" data:(0x)");
            printf_len += 9;

            for (int i = 0; i < zcan_transmit_data[0].frame.can_dlc; i++)
            {
                printf(" %02X", zcan_transmit_data[0].frame.data[i]);
		if ((i + 1) % 8 == 0 )
		{
		   printf("\n");
                   printf("%*s", printf_len, "");
        ...
        if (zcan_transmit(m_h_channel[device_index][1], zcan_transmit_data, message_len) == 1)
        {
            printf("Index:%04d  ", message_count);
            message_count++;
            printf("CAN2 TX ID:0x%08X", id);
            if (IS_EFF(zcan_transmit_data[0].frame.can_id) == 0)
            {
                printf(" Standard ");
            }
    ...
    usleep(10000000);
    run_flag = 0;                  // 线程关闭指令。
    pthread_join(thread_id, NULL); // 等待线程关闭。
    usleep(100000);                // 延时100ms。

    zcan_reset_can(m_h_channel[device_index][0]); // 复位CAN1通道。
    usleep(100000);                               // 延时100ms。

    zcan_reset_can(m_h_channel[device_index][1]); // 复位CAN2通道。
