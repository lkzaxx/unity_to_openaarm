...
{
    if(INVALID_DEVICE_HANDLE != m_dev)//device opened already
        return false;
    m_dev = ZCAN_OpenDevice(device_type,device_index,reserved);
    if(INVALID_DEVICE_HANDLE == m_dev)
        return false;
    devPtr = GetIProperty(m_dev);
    if(devPtr == nullptr)//Failed to set the specified path property
    {
...
{
    if(1 != devPtr->SetValue("0/filter_clear","0"))//filter clear
        return false;
    ...
    if(1 != devPtr->SetValue("1/filter_end",endID.toStdString().c_str()))
        return false;
    if(1 != devPtr->SetValue("0/filter_ack","0"))//filter active
        return false;
    if(1 != devPtr->SetValue("1/filter_ack","0"))
        return false;
    //If you want to set multiple filters, you can set multiple filters between clearing the filter and the filter takes effect
...
{
    if(ZCAN_StartCAN(m_channel1) != 1)//init ch1
        return false;
    if(ZCAN_StartCAN(m_channel2) != 1)//init ch2
        return false;
    return true;
}
...
bool CANThread::sendData(UINT id,UINT frame_type_index,UINT protocol_index,UINT canfd_exp_index,UINT channel,const char *data,UINT len)
{
    ...
    if (0 == protocol_index) //can
    {
        ...
        can_data->transmit_type = 1;//single send
    ...
    else //canfd
    {
        ...
        canfd_data->transmit_type = 1;//single send
...
//0.reset device, then goto step 4
bool CANThread::reSetCAN()
{
    if(ZCAN_ResetCAN(m_channel1) != 1)//reset ch1
        return false;
    if(ZCAN_ResetCAN(m_channel2) != 1)//reset ch2
        return false;
    return true;
}
