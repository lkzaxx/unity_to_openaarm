            ...
            while (m_bStart)
            {
                lock (locker)
                {
                    ...
                    if (len > 0)
                    {
                        ...
                        for (int i = 0; i < len; ++i)
                        {
                            can_data[i] = (ZCAN_Receive_Data)Marshal.PtrToStructure(
                                (IntPtr)(ptr+i*size), typeof(ZCAN_Receive_Data));
                        }
                        OnRecvCANDataEvent(can_data, len,0);
                    ...
                    if (len > 0)
                    {
                        ...
                        for (int i = 0; i < len; ++i)
                        {
                            canfd_data[i] = (ZCAN_ReceiveFD_Data)Marshal.PtrToStructure(
                                (IntPtr)(ptr+i*size), typeof(ZCAN_ReceiveFD_Data));
                        }
                        OnRecvFDDataEvent(canfd_data, len,0);
                        Marshal.FreeHGlobal(ptr);
                    }
                    ...
                    if (len > 0)
                    {
                        ...
                        OnRecvCANDataEvent(can_data, len, 1);
                        Marshal.FreeHGlobal(ptr);
                    }
                    ...
                    if (len > 0)
                    {
                        ...
                        OnRecvFDDataEvent(canfd_data, len, 1);
                        Marshal.FreeHGlobal(ptr);
                    }
                }
