#include "canfdthread.h"
#include <QTime>
#include <QCoreApplication>
#include <QMetaType>
#include <string.h>

CANFDThread::CANFDThread()
{
    stopped = false;
...
void CANFDThread::stop()
{
    stopped = true;
}

//1.打开设备
bool CANFDThread::openDevice(UINT device_type, UINT device_index, UINT reserved)
{
...
//2.设置CANFD标准
bool CANFDThread::setCANFDStandard(UINT standard)
{
...
//3.设置波特率
bool CANFDThread::setBaudrateFD(UINT rate1,UINT rate2)
{
...
bool CANFDThread::setCustomBaudrateFD(QString rate)
{
...
//5.使能终端电阻
bool CANFDThread::setResistanceEnable(UINT enable)
{
...
//6.设置滤波 可不设置
bool CANFDThread::setFilter(UINT filterMode,QString startID,QString endID)
{
...
uint CANFDThread::MakeCanId(uint id, int eff, int rtr, int err)//1:extend frame 0:standard frame
{
...
//8-1.发送数据
bool CANFDThread::sendData(UINT id,UINT frame_type_index,UINT protocol_index,UINT canfd_exp_index,UINT channel,const char *data,UINT len)
{
    UINT result; //发送的帧数
...
void CANFDThread::sleep(unsigned int msec)
{
