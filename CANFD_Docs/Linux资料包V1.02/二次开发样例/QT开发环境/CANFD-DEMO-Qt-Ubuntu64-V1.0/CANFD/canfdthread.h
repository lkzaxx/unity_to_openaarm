#ifndef CANFDTHREAD_H
#define CANFDTHREAD_H

#include <QThread>
#include <ControlCANFD.h>
#include <QDebug>

class CANFDThread:public QThread
{
    Q_OBJECT
public:
    CANFDThread();
...
signals:
    //8-2.接收数据
    void recvedCANData(ZCAN_Receive_Data *recvCANData,UINT frameCount,UINT channel);
    void recvedCANFDData(ZCAN_ReceiveFD_Data *recvCANFDData,UINT frameCount,UINT channel);
